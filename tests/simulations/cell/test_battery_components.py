...
class testElectrode(unittest.TestCase):
    ...
    def OCP_ref_p(self, SOC: float) -> float:
    ...
    def dOCPdT_p(self, SOC: float) -> float:
    ...
    def OCP_ref_n(self, SOC: float) -> float:
    ...
    def dOCPdT_n(self, SOC: float) -> float:
    ...
    def test_properties(self) -> None:
        electrode: bc.Electrode = bc.Electrode(L=self.L, A=self.A, kappa=self.kappa, epsilon=self.epsilon, max_conc=self.max_conc, R=self.R, S=self.S,
                                               ...
                                               brugg=self.brugg, SOC=self.SOC, T=self.T, func_OCP=self.OCP_ref_p, func_dOCPdT=self.dOCPdT_p)
    ...
    def test_method_ocp(self) -> None:
        ...
        electrode: bc.Electrode = bc.Electrode(L=self.L, A=self.A, kappa=self.kappa, epsilon=self.epsilon, max_conc=self.max_conc, R=self.R, S=self.S,
                                               T_ref=self.T_ref, D_ref=self.D_ref, k_ref=self.k_ref, Ea_D=self.Ea_D, Ea_R=self.Ea_R, alpha=0.5,
                                               brugg=self.brugg, SOC=self.SOC, T=self.T, func_OCP=self.OCP_ref_p, func_dOCPdT=self.dOCPdT_p)
        self.assertEqual(self.OCP_ref_p(SOC=self.SOC), electrode.ocp())

        # Now change the soc
        soc_new: float = 0.5
        electrode.soc = soc_new
        self.assertEqual(self.OCP_ref_p(SOC=soc_new), electrode.ocp())

    def test_method_docpdT(self) -> None:
        electrode: bc.Electrode = bc.Electrode(L=self.L, A=self.A, kappa=self.kappa, epsilon=self.epsilon, max_conc=self.max_conc, R=self.R, S=self.S,
                                               T_ref=self.T_ref, D_ref=self.D_ref, k_ref=self.k_ref, Ea_D=self.Ea_D, Ea_R=self.Ea_R, alpha=0.5,
                                               brugg=self.brugg, SOC=self.SOC, T=self.T, func_OCP=self.OCP_ref_p, func_dOCPdT=self.dOCPdT_p)
        self.assertEqual(self.dOCPdT_p(SOC=self.SOC), electrode.docpdT())
