"""
Contains classes and functionailities for performing computational intelligence algorithms. 
...
class GA:
    def __init__(self, n_chromosomes: float, bounds: np.ndarray, obj_func: Callable, n_pool: int, n_elite: int, 
                 n_generations: int, mutating_factor: float = 0.8):
        """
    ...
    def calc_fitness(self, population) -> np.ndarray:
        """
    ...
    def sorting(self, population, fitness_array) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def mating_population(self, population, fitness_array) -> np.ndarray:
    ...
    def create_new_population(self, mating_population) -> np.ndarray:
