...
int main()
{
    ...
    NElectrode elec_n(L_n, A_n, kappa_n, epsilon_n, max_conc_n, R_n, S_n,
                      T_ref_n, D_ref_n, k_ref_n, Ea_D_n, Ea_R_n, alpha_n,
                      brugg_n, soc_n, temp, OCP_ref_n_, dOCPdT_n_);
    PElectrode elec_p(L_p, A_p, kappa_p, epsilon_p, max_conc_p, R_p, S_p,
                      T_ref_p, D_ref_p, k_ref_p, Ea_D_p, Ea_R_p, alpha_p,
                      brugg_p, soc_p, temp, OCP_ref_p_, dOCPdT_p_);
    Electrolyte electrolyte(c_init_e, L_e, kappa_e, epsilon_e, brugg_e);
    BatteryCell b_cell(elec_p, elec_n, electrolyte, rho, Vol, C_p, h, A, cap, V_max, V_min, R_cell);

    Discharge cycler(discharge_current, V_min, SOC_lib_min, SOC_lib);

    BatterySolver *solver{new BatterySolver(b_cell, true, true, "poly")};  // creates an solver instance in the free store.
    Solution *sol{new Solution{solver->solve(cycler)}};  // creates a Solution instace in the free store.
