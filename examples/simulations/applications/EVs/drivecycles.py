...
EV_DRIVECYCLES_DIR: str = os.path.join(pathlib.Path(
    __file__).parent.parent.parent.parent.parent.__str__(), "bmslogic", "parameter_sets", "application", "EV", "drivecycles")
print(EV_DRIVECYCLES_DIR)

df_us06: pd.DataFrame = pd.read_csv(os.path.join(EV_DRIVECYCLES_DIR, "us06.csv"))
t_us06: np.ndarray = df_us06['Test Time, secs'].to_numpy()
v_us06: np.ndarray = df_us06['Target Speed, mph'].to_numpy()

df_hwfet: pd.DataFrame = pd.read_csv(os.path.join(EV_DRIVECYCLES_DIR, "hwfet.csv"))
t_hwfet: np.ndarray = df_hwfet['Test Time, secs'].to_numpy()
v_hwfet: np.ndarray = df_hwfet['Target Speed, mph'].to_numpy()

df_udds: pd.DataFrame = pd.read_csv(os.path.join(EV_DRIVECYCLES_DIR, "udds.csv"))
t_udds: np.ndarray = df_udds['Test Time, secs'].to_numpy()
v_udds: np.ndarray = df_udds['Target Speed, mph'].to_numpy()

df_nycc: pd.DataFrame = pd.read_csv(os.path.join(EV_DRIVECYCLES_DIR, "nycc.csv"))
t_nycc: np.ndarray = df_nycc['Test Time, secs'].to_numpy()
v_nycc: np.ndarray = df_nycc['Target Speed, mph'].to_numpy()

df_ftp: pd.DataFrame = pd.read_csv(os.path.join(EV_DRIVECYCLES_DIR, "ftp.csv"))
t_ftp: np.ndarray = df_ftp['Test Time, secs'].to_numpy()
v_ftp: np.ndarray = df_ftp['Target Speed, mph'].to_numpy()

df_bcdc: pd.DataFrame = pd.read_csv(os.path.join(EV_DRIVECYCLES_DIR, "bcdc.csv"))
